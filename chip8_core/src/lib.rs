...
fn execute(&mut self, op: u16) {
  ...
  match (digit1, digit2, digit3, digit4) {
    ...
    // Set V register (VX) == second digit to value given NN
    (6, _, _, _) => {
      let x = digit2 as usize;
      let nn = (op & 0xFF) as u8;
      self.v_reg[x] = nn;
    ...
    // Adds given value tor VX register
    (7, _, _, _) => {
      let x = digit2 as usize;
      let nn = (op & 0xFF) as u8;
    ...
    // VX = VY (similar ot VX = NN but source value from VY register)
    (8, _, _, 0) => {
